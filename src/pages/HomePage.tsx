...
export const HomePage = () => {
  ...
  console.log("render number is : ", rerender);
  rerender++;
  console.log("movie is currently:", movieStatus);
  console.log(movies);
  ...
  /*if (!hasTrending || !hasPopular || !hasUpcoming) {
    return (
  ...
  let renderMedia = () => {
    if (
      movieStatus === "pending" /*&&
      (!hasPopular || !hasTrending || !hasPopular)*/
    ) {
      return (
        <div className="loading-spinner__home">
          <LoadingSpinner />
    ...
    if (!hasTrending || !hasPopular || !hasUpcoming) {
      const fetchAndUpdate = async function () {
        ...
        let results: any = await dispatch(
          fetchTrendingMovies({
            url: mediaTrendingUrl("movie", "week"),
            category: "trending" as MediaCategories,
          })
        ); // ???
        // console.log(results);
